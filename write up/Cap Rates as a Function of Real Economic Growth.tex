...
Those subtleties are why we further examine precision, recall, and specificity. Precision (TP/(TP+FP)) is the ratio of correctly predicted positive observations to total predicted positive observations. The national model achieves 50\% precision, meaning half the time a signal is generated, it is accurate. This seems useless at first blush, but consider that if a signal is rarely generated, then 50\% accuracy would be good reason to take heed. However if a signal is constantly generated, then 50\% accuracy is like a constant fire alarm. The 
