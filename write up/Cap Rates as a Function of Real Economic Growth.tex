...
To establish the validity of the model for predictive power, we trained five separate models (Year = 2015, 2016....2020) which were given historical data and then asked to predict the next years' (Years 2016-2021, 2017-2021,...2021) cap rate expansions and contractions based on the signal series outlined above. 

This was done once for the national model and once for twice for the MSA model. In the first, all MSA models are trained and specified the same. In the second, each MSA model is trained and specified individually. The model was not given the MSA name as a dummy variable.  

Then, to establish the validity of the models for explanatory power, we trained them using all time periods available (in-sample).
...
Accuracy (TP+TN/(TP+FP+TN+FN)) can be understood as the portion of time the model correctly identified a cap rate expansion or contraction. At the national level,cap rate expansion has happened 29.5\% of quarters since 2000. The model then is more accurate than forecasting constant cap rate contraction, which would have yielded an accuracy of 70.5\%. This is not true at the MSA level, where cap rates have expanded 27\% of the time (compressed 73\% of the time) and the model yielded only 62\% accuracy. That is, it would have done better by forecasting constant cap rate compression (yielding an accuracy of 73\%. The individually trained MSA model was more accurate, but still did not achieve an accuracy in excess of the natural occurance of cap rate compressions.

Those subtleties are why we further examine precision, recall, and specificity. Precision (TP/(TP+FP)) is the ratio of correctly predicted positive observations to total predicted positive observations. The national model achieves 50\% precision, meaning half the time a signal is generated, it is accurate. The jointly trained MSA model has weak precision. And the individually trained MSA model has precision in between the two. While the precision of the MSA models seems underwhelming, it must be contextualized in the recall.

Recall (TP/(TP+FN)) is the ratio of correctly identified cap rate expansions to the total number of cap rate expansions. In many MSAs there were only three or six years of cap rate expansions. Compare this to the national model which had far more quarters of cap rate expansion. The recall of the national model was high at 0.6, meaning 60\% of the times when cap rates expanded, the model correctly forecasted them. The recall of the individually trained MSA model was particularly noteworthy, averaging 67\%. Two thirds of the time a cap rate expansion was coming, the individually trained MSA models were able to identify this based on the interaction between inflation and GDP. 
...
\subsubsection{Individually Trained MSA Model}
\csvautotabular{"C:/Users/matth/Documents/GitHub/inflation_cr_expansion/output/individual_msa_level_accuracy.csv"}
...
The MSA model that was jointly trained was relatively weak, which is likely owing to two factors. First, there is simply not enough data available. There were only 20 years of GDP and CPI data available consistently, and they were only reported annually. The model, however should not be disregarded, as data is becoming more widely distributed, with greater frequency, and with higher accuracy. It is not unreasonable to think that a singularly trained MSA model would have power in forecasting cap rates as data proliferates. The second reason the jointly trained MSA model underperformed is the uniqueness of individual MSAs. Of course inflation in Houston (an energy-producing city) would have a different impact than the same inflation in Chicago. Chicago is a city whose cold weather makes it energy dependent and subject to the costs of energy and their inflation. Houston is a city whose hot weather makes it energy-dependant, but it also benefits from higher energy prices, as its primary industry (oil) reaps and distributes benefits of those higher prices. That will eventually be reflected in its GDP, but in a much different way than Chicago, so it would make sense to train these models with their own data solely, and an unique set of smoothing and lags. We can see the benefit of this by looking at the individually trained MSA model.

The individually trained MSA model performed nearly as well as the national model, suggesting that a substitute for ample data is a more specified model. By optimizing the lag and trailing average parameters of each MSA model to a setting that specifically suited the MSA's uniqueness, accurate models were generated. 
...
\subsubsection{Outperformance of Implemented Strategy vs Buy-and-Hold}
\csvautotabular{"C:/Users/matth/Documents/GitHub/inflation_cr_expansion/output/outperformance.csv"}
